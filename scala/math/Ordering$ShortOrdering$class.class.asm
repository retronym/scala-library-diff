// class version 50.0 (50)
// access flags 0x421
public abstract class scala/math/Ordering$ShortOrdering$class {

  // access flags 0x609
  public static abstract INNERCLASS scala/math/Ordering$ShortOrdering scala/math/Ordering ShortOrdering
  // access flags 0x409
  public static abstract INNERCLASS scala/math/Ordering$ShortOrdering$class scala/math/Ordering ShortOrdering$class

  // access flags 0x9
  public static $init$(Lscala/math/Ordering$ShortOrdering;)V
    RETURN
    MAXSTACK = 0
    MAXLOCALS = 1

  // access flags 0x9
  public static compare(Lscala/math/Ordering$ShortOrdering;SS)I
    ILOAD 1
    ILOAD 2
    ISUB
    IRETURN
    MAXSTACK = 2
    MAXLOCALS = 3
}
